...
impl backend::GraphicsBackend for Backend {
            ...
            Backend::Vulkan(b) => {
                ...
                match options.render_target {
                    RenderTarget::Windowed(window_handle, display_handle) => {
                        b.instance_create_surface(window_handle, display_handle)
                            .map_err(|e| anyhow::Error::msg(e.to_string()))?;
                        b.pick_physical_device_and_queues();
                        Ok(())
                    }
    ...
    fn resume(&mut self, options: &BackendOptions) {
        match self {
            Backend::Vulkan(v) => match options.render_target {
                RenderTarget::Headless => {}
                RenderTarget::Windowed(window_handle, display_handle) => {
                    v.instance_create_surface(window_handle, display_handle)
                        .unwrap();
                }
            },
            _ => {
                todo!("resume for other backend")
            }
        }
    }
}
