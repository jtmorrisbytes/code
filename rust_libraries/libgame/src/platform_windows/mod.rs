...
impl WidePtr<'_> {
    pub fn as_ptr(&self) -> PCWSTR{ self.ptr }
}
...
impl WideString {
    ...
    pub fn from_os_str_like<S: AsRef<OsStr>>(s:S) -> Self {
        Self { buf: s.as_ref().encode_wide().chain(std::iter::once(0)).collect() }
    }
    pub fn as_wideptr<'a>(&'a self) -> WidePtr<'a> {
        WidePtr{ptr: PCWSTR(self.buf.as_ptr()),_marker:PhantomData}
    }
}


impl<T> From<T> for WideString where T: AsRef<OsStr> {
