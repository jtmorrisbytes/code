...
#[cfg(not(all(target_arch = "wasm32", target_os = "unknown")))]
pub async fn insert_bank_of_america_transactions_into_transactions_table_for_user(
    pool: PgPool,
...
pub async fn view_summary_of_transactions_group_by_name_vendor_and_category(
    ...
    */
    ...
    let _transactions: Vec<Transaction> = query.clone().get_results(&mut *connection).await?;
